...
class Pargammon(object):
    ...
    def jugar(self, txt_jugada: str)-> str |None:
        ...
        if len(txt_jugada) != self.D: # Si la longitud no es de D caracteres ERROR J1
        ...
        for letra in txt_jugada: #Almacenamos en una lista las letras que no aparecen en el tablero excepto la @
            if letra.upper() not in self.tablero and letra != '@':
        ...
        jugadas_posibles = self.jugadas_posibles(self.tablero,self.dados) #Obtnemos una lista con las jugadas posibles
        ...
        for i, letra in enumerate(txt_jugada):
            if letra != '@': # si la letra no es @ pasamos al siguiente dado
                error = self.mover_ficha(letra, self.dados[i])
    ...
    def mover_ficha(self, origen, dado) -> str |None:
        ...
        if origen.isupper(): #comprobamos que sea mayuscula la letra de origen
            ...
            if ord(destino) == 65 + self.N: #Si la letra de destino es la N+1 sacamos la ficha
                ...
                return None
        else: #si es minuscula el desplazamiento del dado es hacia atras
            origen = origen.upper()
            destino = chr(ord(origen.upper())-dado)
    ...
    def verificar_movimiento_valido(self, origen, dado) -> str |None:
        ...
        columna_origen = self.tablero[origen.upper()] #sacamos la columna de origen
        if not columna_origen or columna_origen[-1] != self.FICHAS[self.turnos]: 
            ...
            return f"ERROR J2-M2: Columna de origen {origen.upper()} no tiene fichas del jugador."
        if origen.isupper():
            ...
            origen = origen.upper()
            destino = ord(origen) - dado
            if destino <65:
                return f"ERROR J2-M3: Movimiento {origen.upper()} -> {chr(destino)}, columna de destino fuera de rango."
        ...
        for letra in tablero: 
            ...
            if columna and columna[-1] == self.FICHAS[self.turnos]:  # Si la columna tiene una ficha del jugador actual
                ...
                if destino >='A':
                    ...
                    if len(columna_destino) == 0 or \
                        (len(columna_destino) == 1 and columna_destino[-1] != self.FICHAS[self.turnos]) or \
                        (columna_destino[-1] == self.FICHAS[self.turnos]): 
                        ...
                        # Llamamos recursivamente para hacer la siguiente jugada con los dados restantes
                        nuevas_jugadas = self.jugadas_posibles(nuevo_tablero, dados[1:], jugada_actual + letra.lower())
                        posibles.extend(nuevas_jugadas) #Almacenamos la jugada completada en la lista de posibles
    ...
    def jugada_maquina_tonta(self)->str:
        ...
        lista_jugadas = self.jugadas_posibles(self.tablero, self.dados) #Carga la lista de jugadas posibles del jugador actual
    ...
    def jugada_maquina_lista(self)->str:
        ...
        jugadas_posibles = self.jugadas_posibles(self.tablero,self.dados) #Carga la lista de jugadas posibles del jugador actual
    ...
    def mejor_jugada(self)->list:
        ...
        lista_jugadas = self.jugadas_posibles(self.tablero, self.dados)
            ...
            for i, letra in enumerate(jugada):
                if letra != '@':  # Solo movemos si no es '@'
                    ...
                    if letra.isupper():
                        ...
                        letra = letra.upper()
                        destino = chr(ord(letra.upper())-dado)
...
def main():
    ...
    while (juego.cambiar_turno() == False): #bucle principal del juego
        ...
        lista = juego.jugadas_posibles(juego.tablero,juego.dados)
        valores = juego.mejor_jugada()
        lista,valores = juego.ordenar_jugadas(lista,valores)
        #print(len(lista),flush=True)
        ...
        # Jugada de la maquina lista    
            ...
            print(f"Jugada: {jugada}") #La mostramos por pantalla
            print()
            resultado = juego.jugar(jugada) #Llamamos a jugar con esa jugada (si esta bien hecho la jugada será siempre valida)
            if resultado != None:
                print(resultado)
        else: #Jugada del humano
            while True:
                jugada = input(f"Jugada: ").strip()  # Pedir jugada
